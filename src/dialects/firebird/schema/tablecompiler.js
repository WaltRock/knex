...
assign(TableCompiler_Firebird.prototype, {
  ...
  getFKRefs(runner) {
    ...
    const sql = 'SELECT KCU.CONSTRAINT_NAME, KCU.TABLE_NAME, KCU.COLUMN_NAME, ' +
            ...
            ' USING(CONSTRAINT_NAME)' + 'WHERE KCU.REFERENCED_TABLE_NAME = ' +
  ...
  index(columns, indexName) {
    ...
    this.pushQuery('alter table ' +
            this.tableName() + " add index " +
            indexName + "(" + this.formatter.columnize(columns) + ")");
  },

  primary(columns, constraintName) {
    this.pushQuery('ALTER TABLE ' + this.tableName() + ' ADD CONSTRAINT ' + constraintName +
